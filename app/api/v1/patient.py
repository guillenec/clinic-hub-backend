...
@router.get("/me/vitalsigns", response_model=list[dict], dependencies=[Depends(require_roles(RoleEnum.patient))])
async def get_patient_vitals(current: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    # Obtener al paciente asociado al usuario actual usando el user_id
    pt = await db.execute(select(Patient).where(Patient.user_id == current.id))
    pt = pt.scalar_one_or_none()
    if not pt:
        raise HTTPException(status_code=404, detail="Paciente no encontrado")
    
    # Obtener los signos vitales del paciente
    vital_signs = await db.execute(select(Vital).where(Vital.patient_id == pt.id))
    vital_signs = vital_signs.scalars().all()

    # Si no tiene signos vitales, lanza un error
    if not vital_signs:
        raise HTTPException(status_code=404, detail="No se encontraron signos vitales para el paciente")
    
    # Devolver los signos vitales en el formato adecuado
    return [{"id": vs.id ,"metric": vs.metric, "value": vs.value, "status": vs.status, "date": vs.date.isoformat()} for vs in vital_signs]
